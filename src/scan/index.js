...
export async function scanNetwork({
  ...
  emit("phase", {
    phase: "survey",
    message: "Inspecting local interfaces and routes",
  });
  ...
  for (const s of survey.subnets) {
    if (s.isTunnel)
      warnings.push(
        `Skipped ${s.name} (${s.cidr}) — looks like a VPN/tunnel interface.`,
      );
    else if (s.tooLarge)
      warnings.push(
        `Skipped ${s.name} (${s.cidr}) — ${s.size} addresses exceeds the ${maxHosts} host limit.`,
      );
  }
  ...
  if (scanTargets.length === 0) {
    warnings.push(
      "No scannable IPv4 subnet found. Pass an explicit target, e.g. --target 192.168.1.0/24",
    );
  }

  const selfIps = new Set(survey.subnets.map((s) => s.address));
  const gatewayIps = new Set(
    survey.subnets.map((s) => s.gateway).filter(Boolean),
  );
  ...
  if (useSudo && !nmap.isPrivileged() && !(await nmap.sudoAvailable())) {
    useSudo = false;
    warnings.push(
      "sudo -n has no cached credentials, so the scan ran unprivileged. Run `sudo -v` in a terminal first, then scan again.",
    );
  }
  ...
  if (haveNmap && scanTargets.length) {
    ...
    if (!privileged) {
      warnings.push(
        "Running nmap without root: ARP host discovery, SYN scans and OS detection are unavailable. Re-run with --sudo for a fuller map.",
      );
    }
    ...
    if (profile !== "quick" && hostMap.size) {
      emit("phase", {
        phase: "probe",
        message: `Probing ports/services on ${hostMap.size} hosts (${profile})`,
      });
      ...
      const openTotal = [...hostMap.values()].reduce(
        (n, h) => n + (h.ports?.length || 0),
        0,
      );
      if (openTotal === 0) {
        warnings.push(
          "The port probe found nothing open. Unprivileged connect scans are slow over wireless and hosts can run out their time budget — try --sudo for a SYN scan, or the quick profile if the device list is all you need.",
        );
      }
    }
    ...
    if (!haveNmap)
      warnings.push(
        "nmap not found on PATH — using the built-in ping sweep. Installing nmap adds ARP discovery, service versions, OS detection and a much larger MAC vendor database.",
      );
    ...
    emit("phase", {
      phase: "discover-done",
      message: `${hostMap.size} hosts responded`,
      count: hostMap.size,
    });
  }
  ...
  emit("phase", {
    phase: "enrich",
    message: "Reading ARP cache and resolving vendors",
  });
  ...
  for (const s of survey.subnets) {
    ...
    for (const ip of [s.address, s.gateway]) {
      ...
      hostMap.set(ip, {
        ip,
        mac: ip === s.address ? s.mac : arp.get(ip)?.mac || null,
        ports: [],
        hostnames: [],
        hostname: ip === s.address ? survey.hostname : null,
        reason: ip === s.address ? "local-interface" : "default-route",
        rttMs: null,
        hops: ip === s.address ? 0 : 1,
      });
    }
  }
  ...
  if (resolveHostnames && hostMap.size) {
    emit("phase", {
      phase: "names",
      message: `Resolving names for ${hostMap.size} hosts (mDNS, NetBIOS, DNS)`,
    });
    ...
    emit("phase", {
      phase: "names-done",
      message: `Named ${named} additional host${named === 1 ? "" : "s"}`,
    });
  }

  const portsKnown = profile !== "quick";
  const devices = [...hostMap.values()]
    .map((h) => {
      const mac = normalizeMac(h.mac);
      const vendor = h.vendor || vendorOf(mac);
      const subnet = survey.subnets.find((s) =>
        parseCidr(s.cidr)?.contains(h.ip),
      );
  ...
  if (doTraceroute) {
    emit("phase", {
      phase: "traceroute",
      message: `Tracing the route to ${tracerouteTarget}`,
    });
    uplink = await traceroute(tracerouteTarget);
    if (uplink.unavailable)
      warnings.push("traceroute not available — the uplink path is omitted.");
  }
  ...
  emit("done", {
    message: `Scan complete: ${devices.length} devices in ${((Date.now() - startedAt) / 1000).toFixed(1)}s`,
  });
  return model;
}
